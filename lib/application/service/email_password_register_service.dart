...
class EmailPasswordRegisterService
  ...
  Future<void> register(EmailPasswordRegister userRegister) async {
    ...
    if (userRegister.email.isEmpty) {
      throw FormatException('Email không được để trống');
    }
    if (!RegExp(r'^[^@]+@[^@]+\.[^@]+').hasMatch(userRegister.email)) {
      throw FormatException('Email không hợp lệ');
    }

    // Kiểm tra mật khẩu hợp lệ
    if (userRegister.password.isEmpty) {
      throw FormatException('Mật khẩu không được để trống');
    }
    if (userRegister.password.length < 8) {
      throw FormatException('Mật khẩu phải có ít nhất 8 ký tự');
    }
    if (!RegExp(r'[A-Z]').hasMatch(userRegister.password)) {
      throw FormatException('Mật khẩu phải chứa ít nhất một chữ hoa');
    }
    if (!RegExp(r'[a-z]').hasMatch(userRegister.password)) {
      throw FormatException('Mật khẩu phải chứa ít nhất một chữ thường');
    }
    if (!RegExp(r'[0-9]').hasMatch(userRegister.password)) {
      throw FormatException('Mật khẩu phải chứa ít nhất một số');
    }
    if (!RegExp(r'[!@#$%^&*(),.?":{}|<>]').hasMatch(userRegister.password)) {
      throw FormatException('Mật khẩu phải chứa ít nhất một ký tự đặc biệt');
    }
